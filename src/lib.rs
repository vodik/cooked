...
impl<'e> FromLisp<'e> for emu::Rgb {
    /// A colour as `color-values' gives it: a list of three channels, sixteen bits each.
    ///
    /// A channel outside a `u16` is clamped rather than refused. Emacs' own colours are
    /// exactly that wide, so a wider number is a caller that scaled something twice, and a
    /// colour slightly wrong is a better answer to a child's query than a signal in the
    /// middle of a theme change. A list that is not three channels is refused instead,
    /// there being no colour to guess at.
    ///
    /// Nil arrives as `None` through the blanket impl for `Option`, which is how
    /// `cooked--set-palette' takes a slot Emacs could not resolve.
    fn from_lisp(env: &Env<'e>, v: Value<'e>) -> Result<Self> {
        let channels = each(*env, v, |channel| {
            Ok(env.from_lisp::<i64>(channel)?.clamp(0, i64::from(u16::MAX)) as u16)
        })?;
        match channels[..] {
            [r, g, b] => Ok(emu::Rgb { r, g, b }),
            _ => Err(env.signal_wrong_type("cooked-color-values-p", v)),
        }
    }
}

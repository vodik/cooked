...
/// A decoded sixel: RGBA pixels and the size they make up.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct Bitmap {
    pub width: u32,
    pub height: u32,
    /// Row-major RGBA, `4 * width * height` bytes.
    pub pixels: Vec<u8>,
}
...
fn measure(body: &[u8]) -> Option<(u32, u32)> {
    ...
    // Multiplied as `usize` on a 64-bit target, so two `u32`s cannot wrap it.
                ...
                for _ in 0..repeat {
                    ...
                    for row in 0..6 {
                        if bits & (1 << row) != 0 {
                            let y = 6 * band + row;
                            if y < bitmap.height {
                                let at = 4 * (y as usize * bitmap.width as usize + x as usize);
                                bitmap.pixels[at..at + 4].copy_from_slice(&pen);
                            }
                        }
                    }
                    x += 1;
...
mod tests {
    ...
    fn at(bitmap: &Bitmap, x: u32, y: u32) -> [u8; 4] {
        let i = 4 * (y as usize * bitmap.width as usize + x as usize);
    ...
    fn raster_attributes_set_a_floor_for_the_canvas_not_the_answer() {
        ...
        assert_eq!((bitmap.width, bitmap.height), (10, 12));
        assert_eq!(at(&bitmap, 0, 0), RED);
    ...
    fn a_picture_larger_than_the_cap_is_refused_rather_than_allocated() {
        ...
        assert_eq!((wide.width, wide.height), (2000, 6));
    }

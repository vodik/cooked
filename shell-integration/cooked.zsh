...
compadd() {
  ...
  # call: a completer that offered nothing has no bounds worth having.
  if (( ! __cooked_prefix && ! __cooked_suffix )); then
    __cooked_prefix=${#PREFIX}
    __cooked_suffix=${#SUFFIX}
  fi

  local -i n=1
  for arg in "${caught[@]}"; do
    __cooked_matches+=( "$apre$hpre$arg" )

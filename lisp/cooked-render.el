...
Reading `window-end' with no UPDATE for that check is deliberate.  Without
UPDATE it answers Z minus a count fixed
...
and `cooked-pin-follows-a-tail-that-grows-and-shrinks-with-no-redisplay-between'
holds it to that: it grows and shrinks the tail across two drains with no
redisplay run between them and fails if this reads a redisplay behind."
...
(defun cooked--apply-rows (update viewport)
  "Write UPDATE's damaged rows, carrying VIEWPORT's relocations across them.
...
five with the shifts moved after the rows."
  ...
  (cooked--trim-scrollback)
  ;; After the trim, because what this reads is a flag redisplay sets about the
  ;; buffer's lines, and the trim is the last thing that changes them.  Outside
  ;; the bindings above only by being down here; it edits nothing.
  ;; One `long-line-optimizations-p' per

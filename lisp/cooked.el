...
Four of the five are cheap: accessors, none of which selects a window, measured
at half a microsecond apiece.  The `font\=' frame parameter is the expensive one
and is here anyway.  It names the font outright, where the rest only describe
its metrics, and without it a font swapped for another of exactly the same cell
size leaves the memo answering for the outgoing font -- `frame-char-width\=' and
`frame-char-height\=' cannot see that swap, and every row already seen to fit
keeps the previous font's verdict until something else moves.

It costs 21.4us to read on pgtk, being computed from the frame's font object
rather than stored, which is more than everything else here put together.  That
was the whole of the case against it while this was built per row: 21us a row is
not a cost this guard may carry, and the hole it left was cosmetic-at-worst (see
`cooked--wrap-cache\=').  The stamp is now built once per drain, in
`cooked--render-rows\=', and threaded in by way of `cooked--wrap-cache\=' -- so
the same 21.4us is one read against a drain measured at 590us, and the trade
turns over.  A caller that has not threaded it is answered per row, correctly
and slowly, which is the same fallback `cooked--layout-window\=' has and is not a
path any render takes.
...
and `cooked--wrap-cache\=' is what holds it in between.
...
Why this replaces a blanket \"a graphical frame might do anything\": the thing
that arm was written to defend against was ligatures, and ligatures turn out not
to be able to cause the failure.  Measured across Noto Sans Mono, Iosevka Fixed
SS10, Adwaita Mono and the generic `monospace\=', with 27 ligature sequences
(`->\=', `=>\=', `===\=', `www\=', `ffi\=' and the rest), both under Emacs'
default composition and with ligatures forced on as ligature.el does: 0 of 27
rendered at anything other than the nominal `frame-char-width\=' * length, in
any of the eight combinations.  A monospace font draws a ligature inside the
cells its characters already occupied -- that is what makes it monospace -- and
this guard only ever trims a row that renders *wider* than nominal, since that
is what softwrapping means.  The control in the same measurement was Iosevka
Aile, a quasi-proportional face, where 26 of 27 came out wrong (`=>\=' 18px
nominal against 20px actual, `www\=' 27px against 39px).

Which is the real hazard, and the one this measures: a face whose font is not
fixed pitch at all -- `buffer-face-mode\=', a `:family\=' on the default face, a
fallback font chosen for a character the primary font lacks.  ASCII in a
proportional face is wider than the grid assumed, exactly as a mismeasured wide
character is, and it is the case vterm's README warns about.  It survives here
as a real answer instead of a blanket assumption."
...
one.  There used to be a fallback here, measuring the row's text with
`string-width\=' when WIDTH was nil, for a caller with a row but no block to
have got one from -- taking START and END to have text to measure.  No such
caller ever existed outside a test calling this directly, so both are gone
rather than kept for a case nothing reaches; see the tests for how they pass
WIDTH explicitly now.  A row already wider than `cooked--cols\=' is genuinely
long, scrollback from a wider grid predating a resize, and should soft-wrap
...
byte-per-column reading.  Comes from the same place for the same reason --
Lisp's version of it was a `string-match-p\=' over the row.  FIXED-PITCH is
...
The row that motivates the memo is a border: 248 identical box-drawing
characters, rewritten by a full-screen program on every frame it paints,
arriving as damage 125 times a second because the child really did rewrite it,
and laying out the same way every one of those times.  Measuring it is not free:
`vertical-motion\=' over a row of box glyphs cost 445us of the 545us the whole
...
out of, and it is an argument for the reason WINDOW is one: it is a fact about
the drain and not about the row.  Validating it means building
`cooked--layout-stamp', which was measured at 22.6us when it was only accessors
and costs a further 21.4us now that it names the font -- per row, that is more
than the whole guard is trying to get down to.  `cooked--render-rows' asks once
and threads it in, and the answer cannot change while a render is in progress,
exactly as WINDOW's cannot.  A caller without one is answered per row,
correctly and slowly.

Before those existed this ran the whole way down for every row of every drain
on a graphical frame, because the frame check that was supposed to be the fast
path was the left arm of an `or' and so was unconditionally true there.  On a
248-column frame that was 2.2ms per 45-row drain for ASCII, 5.1ms for CJK and
24.5ms for box drawing -- the last of them more than a whole 60Hz frame, on the
apply path, spent to conclude there was nothing to do.
...
chance when it is displayed.

WINDOW is an argument because the answer is a property of the drain rather than
of the row.  `cooked--layout-window' walks `get-buffer-window-list' and asks
`window-max-chars-per-line' of every window it finds, and that subr does its
work inside `with-selected-window' -- so computing it here meant two
`select-window' round trips per displayed window per rendered row, which is
arbitrary advice running in the middle of a render for an answer that cannot
change while one is in progress.  `cooked--render-rows' computes it once and
hands it down.  See docs/DESIGN.md.

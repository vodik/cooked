...
there is the child's own left arrow, and the core spells it: under DECCKM the
shell reads `ESC O D' for that key, not the `ESC [ D' a hand-written escape
would send, and a line editor that never sees the byte it asked for leaves the
cursor short of where the rest of the line expects it.  The line, the arrows
and KEY go out as one write, composed under the lock that reads the mode; see

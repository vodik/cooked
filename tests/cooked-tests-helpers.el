...
;; `cooked-osc-4-sweep-is-answered-without-waking-lisp' failed under SELECTOR
;; and passed in the full suite, where an earlier test had loaded it.
(cooked--load-module)
              ...
              (when (buffer-live-p buffer)
                (with-current-buffer buffer
                  (when cooked--session
                    ...
                    (cooked--apply
                     (cooked--drain cooked--session cooked-rejoin-wrapped-lines 'promoting)))))))
...
(defmacro cooked-tests--with-filled-screen (&rest body)
  "Run BODY with a 12x20 session `cooked-tests--filled-screen' has filled."
  (declare (indent 0))

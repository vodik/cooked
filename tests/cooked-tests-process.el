...
(ert-deftest cooked-process-gives-the-child-a-tty ()
  "The whole reason the file exists: `isatty' answers yes."
  (cooked-tests-process--with "if [ -t 1 ]; then echo yes; else echo no; fi"
    (should (equal (cooked-tests-process--body) "yes"))))
...
`ansi-color' cannot do this and never could: it is a filter over a stream,
and which of the three is the survivor is a property of the grid."
